...
while : ; do # Loop until root password is correct or Cancel button is pressed
	PASSWD=$(whiptail --title "BashTool_GSerial" --passwordbox "The Tool needs root privileges.\nEnter the root password:" 10 60 3>&1 1>&2 2>&3) # Password screen
...
EOF
	then
		whiptail --title "BashTool_GSerial" --msgbox "Root password verified, the script has obtained permissions" 10 60 0 --cancel-button Exit --ok-button Ok # Correct Password screen
		break 
	else
		whiptail --title "BashTool_GSerial" --msgbox "Invalid password, try again..." 10 60 0 --cancel-button Exit --ok-button Retry # Bad Password screen
	fi
done
